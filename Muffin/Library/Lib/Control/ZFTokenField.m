...
- (void) setViewOnly:(BOOL)readOnly
{
    if (readOnly)
    {
        [self removeTarget:self action:@selector(focusOnTextField) forControlEvents:UIControlEventTouchUpInside];
...
- (void)reloadData
{
    ...
    self.titleText.frame = CGRectMake(5, 5, CGRectGetWidth(self.frame)-20, 20);
    
    if (self.autoHeight)
    {
        if (sz.height != 0 && (sz.height + _marginTop + _marginBottom) != self.frame.size.height)
        {
            CGRect r = self.frame;
            r.size.height = sz.height+ _marginTop + _marginBottom;
            self.frame = r;
            
            if (_delegate)
            {
                if ([_delegate respondsToSelector:@selector(autoSizeChangeInField:frame:)])
                {
                    [_delegate autoSizeChangeInField:self frame:r];
                }
            }
        }
    }
...
- (void) setShowTitleText:(BOOL)showTitleText
{
    _marginTop = _marginTop + 25;
...
- (void)enumerateItemRectsUsingBlock:(void (^)(CGRect itemRect))block
{
    ...
    CGFloat x = _marginLeft, y = _marginTop;
    CGFloat margin = 0;
    ...
    for (UIView *token in self.tokenViews) {
        CGFloat width = MAX(CGRectGetWidth(self.bounds)-(_marginLeft + _marginRight), CGRectGetWidth(token.frame));
        CGFloat tokenWidth = MIN(CGRectGetWidth(self.bounds)-(_marginLeft + _marginRight), CGRectGetWidth(token.frame));
        ...
        if ([token isKindOfClass:[ZFTokenTextField class]]) {
            ...
            if (size.width > CGRectGetWidth(self.bounds) - (_marginLeft + _marginRight)) {
                size.width = CGRectGetWidth(self.bounds)- (_marginLeft + _marginRight);
            }
